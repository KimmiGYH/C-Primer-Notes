- [编译内存](#编译内存)
  ...
  - [对象创建限制在堆或栈](#对象创建限制在堆或栈)
  - [内存对齐](#内存对齐)
  - [类的大小](#类的大小)
  - [什么是内存泄露](#什么是内存泄露)
  - [怎么防止内存泄漏？内存泄漏检测工具的原理？](#怎么防止内存泄漏内存泄漏检测工具的原理)
  - [智能指针有哪几种？智能指针的实现原理？](#智能指针有哪几种智能指针的实现原理)
  - [一个 `unique_ptr` 怎么赋值给另一个 u`nique_ptr` 对象？](#一个-unique_ptr-怎么赋值给另一个-unique_ptr-对象)
  - [使用智能指针会出现什么问题？怎么解决？](#使用智能指针会出现什么问题怎么解决)
- [语言对比](#语言对比)
  - [C++ 11 新特性](#c-11-新特性)
  - [C 和 C++ 的区别](#c-和-c-的区别)
...
- [面向对象](#面向对象)
  ...
  - [如何理解 C++ 是面向对象编程](#如何理解-c-是面向对象编程)
  - [什么是多态？多态如何实现？](#什么是多态多态如何实现)
- [关键字库函数](#关键字库函数)
  - [`sizeof` 和 `strlen` 的区别](#sizeof-和-strlen-的区别)
  - [lambda 表达式（匿名函数）的具体应用和使用场景](#lambda-表达式匿名函数的具体应用和使用场景)
  - [explicit 的作用（如何避免编译器进行隐式类型转换）](#explicit-的作用如何避免编译器进行隐式类型转换)
  - [C 和 C++ static 的区别](#c-和-c-static-的区别)
  - [static 的作用](#static-的作用)
  - [static 在类中使用的注意事项（定义、初始化和使用）](#static-在类中使用的注意事项定义初始化和使用)
  ...
  - [用宏实现比较大小，以及两个数中的最小值](#用宏实现比较大小以及两个数中的最小值)
  - [inline 作用及使用方法](#inline-作用及使用方法)
  - [**`inline`** 函数工作原理](#inline-函数工作原理)
  - [宏定义（define）和内联函数（inline）的区别](#宏定义define和内联函数inline的区别)
  - [`new` 的作用？](#new-的作用)
  - [`new` 和 `malloc` 如何判断是否申请到内存？](#new-和-malloc-如何判断是否申请到内存)
  ...
  - [malloc 的原理？malloc 的底层实现？](#malloc-的原理malloc-的底层实现)
  - [C 和 C++ struct 的区别？](#c-和-c-struct-的区别)
  - [为什么有了 `class` 还保留 `struct`？](#为什么有了-class-还保留-struct)
  - [`struct` 和 `union` 的区别](#struct-和-union-的区别)
  - [`class` 和 `struct` 的异同](#class-和-struct-的异同)
  - [`class` 和 `struct` 的异同](#class-和-struct-的异同-1)
  - [`volatile` 的作用？是否具有原子性，对编译器有什么影响？](#volatile-的作用是否具有原子性对编译器有什么影响)
  - [什么情况下一定要用 `volatile`， 能否和 `const` 一起使用？](#什么情况下一定要用-volatile-能否和-const-一起使用)
  - [返回函数中静态变量的地址会发生什么？](#返回函数中静态变量的地址会发生什么)
  - [`extern "C"` 的作用？](#extern-c-的作用)
  - [`sizeof(1==1)` 在 C 和 C++ 中分别是什么结果？](#sizeof11-在-c-和-c-中分别是什么结果)
  - [`memcpy` 函数的底层原理？](#memcpy-函数的底层原理)
  - [`strcpy` 函数有什么缺陷？](#strcpy-函数有什么缺陷)
  - [auto 类型推导的原理](#auto-类型推导的原理)
- [类相关](#类相关)
  - [什么是虚函数？什么是纯虚函数？](#什么是虚函数什么是纯虚函数)
  - [虚函数和纯虚函数的区别？](#虚函数和纯虚函数的区别)
  - [虚函数的实现机制](#虚函数的实现机制)
  - [单继承和多继承的虚函数表结构](#单继承和多继承的虚函数表结构)
  - [如何禁止构造函数的使用？](#如何禁止构造函数的使用)
  - [什么是类的默认构造函数？](#什么是类的默认构造函数)
  - [构造函数、析构函数是否需要定义成虚函数？为什么？](#构造函数析构函数是否需要定义成虚函数为什么)
  - [如何避免拷贝？](#如何避免拷贝)
  - [如何减少构造函数开销？](#如何减少构造函数开销)
  - [多重继承时会出现什么状况？如何解决？](#多重继承时会出现什么状况如何解决)
  - [空类占多少字节？C++ 编译器会给一个空类自动生成哪些函数？](#空类占多少字节c-编译器会给一个空类自动生成哪些函数)
  - [为什么拷贝构造函数必须为引用？](#为什么拷贝构造函数必须为引用)
  - [C++ 类对象的初始化顺序](#c-类对象的初始化顺序)
  - [如何禁止一个类被实例化？](#如何禁止一个类被实例化)
  - [为什么用成员初始化列表会快一些？](#为什么用成员初始化列表会快一些)
  - [实例化一个对象需要哪几个阶段](#实例化一个对象需要哪几个阶段)
  - [友元函数的作用及使用场景](#友元函数的作用及使用场景)
  - [静态绑定和动态绑定是怎么实现的？](#静态绑定和动态绑定是怎么实现的)
  - [深拷贝和浅拷贝的区别](#深拷贝和浅拷贝的区别)
  - [编译时多态和运行时多态的区别](#编译时多态和运行时多态的区别)
  - [实现一个类成员函数，要求不允许修改类的成员变量？](#实现一个类成员函数要求不允许修改类的成员变量)
  - [如何让类不能被继承？](#如何让类不能被继承)
- [语言特性相关](#语言特性相关)
  - [左值和右值的区别？左值引用和右值引用的区别，如何将左值转换成右值？](#左值和右值的区别左值引用和右值引用的区别如何将左值转换成右值)
  - [`std::move()` 函数的实现原理](#stdmove-函数的实现原理)
  - [什么是指针？指针的大小及用法？](#什么是指针指针的大小及用法)
  - [什么是野指针和悬空指针？](#什么是野指针和悬空指针)
  - [C++ 11 `nullptr` 比 `NULL` 优势](#c-11-nullptr-比-null-优势)
  - [指针和引用的区别？](#指针和引用的区别)
  - [常量指针和指针常量的区别](#常量指针和指针常量的区别)
  - [函数指针和指针函数的区别](#函数指针和指针函数的区别)
  - [强制类型转换有哪几种？](#强制类型转换有哪几种)
  - [如何判断结构体是否相等？能否用 `memcmp` 函数判断结构体相等？](#如何判断结构体是否相等能否用-memcmp-函数判断结构体相等)
  - [参数传递时，值传递、引用传递、指针传递的区别？](#参数传递时值传递引用传递指针传递的区别)
  - [什么是模板？如何实现？](#什么是模板如何实现)
  - [函数模板和类模板的区别？](#函数模板和类模板的区别)
  - [什么是可变参数模板？](#什么是可变参数模板)
  - [什么是模板特化？为什么特化？](#什么是模板特化为什么特化)
  - [`include " "` 和 `include <>` 的区别](#include---和-include--的区别)
  - [`switch` 的 `case` 里为何不能定义变量](#switch-的-case-里为何不能定义变量)
  - [迭代器的作用？](#迭代器的作用)
  - [泛型编程如何实现？](#泛型编程如何实现)
  - [什么是类型萃取？](#什么是类型萃取)
- [设计模式](#设计模式)
  - [了解哪些设计模式？](#了解哪些设计模式)
  - [什么是单例模式？如何实现？应用场景？](#什么是单例模式如何实现应用场景)
  - [什么是工厂模式？如何实现？应用场景？](#什么是工厂模式如何实现应用场景)
  - [什么是观察者模式？如何实现？应用场景？](#什么是观察者模式如何实现应用场景)
  - [参考资料](#参考资料)
...
- **静态建立**：由编译器为对象在**栈空间**上分配内存，直接调用类的构造函数创建对象。例如：`A a`;
- **动态建立**：使用 `new` 关键字在**堆空间上**创建对象，底层首先调用 ``operator new()`` 函数，在堆空间上寻找合适的内存并分配；然后，调用类的构造函数创建对象。例如：`A *p = new A()`;
...
![内存泄漏检测工具的实现原理](C++图库/内存泄漏检测工具的实现原理.png)
...
## 一个 `unique_ptr` 怎么赋值给另一个 u`nique_ptr` 对象？
...
![虚函数_基类的虚函数](C++图库/虚函数_基类的虚函数.png)

派生类的对象虚函数表如下：

![派生类的对象虚函数](C++图库/虚函数_派生类的对象虚函数.png)
...
- `struct` 和 `class` 都可以自定义数据类型，也支持继承操作。
- `struct` 中默认的访问级别是 `public`，默认的继承级别也是 `public`；`class` 中默认的访问级别是 `private`，默认的继承级别也是 `private`。
- 当 `class` 继承 `struct` 或者 `struct` 继承 `class` 时，默认的继承级别取决于 · 或 `struct` 本身， `class`（`private` 继承），`struct`（`public` 继承），即取决于派生类的默认继承级别。
...
![类相关01_基类和派生类的继承](C++图库/类相关01_基类和派生类的继承.png)

基类的虚函数表：

![类相关02_基类的虚函数表](C++图库/类相关02_基类的虚函数表.png)

派生类的虚函数表：

![类相关03_派生类的虚函数表](C++图库/类相关03_派生类的虚函数表.png)
...
![类相关04_类和派生类的继承](C++图库/类相关04_类和派生类的继承.png)

基类的虚函数表：

![类相关05_基类的虚函数表](C++图库/类相关05_基类的虚函数表.png)

派生类的虚函数表：

![类相关06_派生类的虚函数表](C++图库/类相关06_派生类的虚函数表.png)
...
![类相关07_派生类的虚函数表](C++图库/类相关07_派生类的虚函数表.png)
...
![类相关08_基类和派生类的关系](C++图库/类相关08_基类和派生类的关系.png)

派生类的虚函数表：（基类的顺序和声明的顺序一致）

![类相关09_派生类的虚函数表](C++图库/类相关09_派生类的虚函数表.png)
...
![类相关10_基类和派生类的关系](C++图库/类相关10_基类和派生类的关系.png)

派生类的虚函数表：

![类相关11_派生类的虚函数表](C++图库/类相关11_派生类的虚函数表.png)
...
![类相关12_上述程序的继承关系如下：（菱形继承）](C++图库/类相关12_上述程序的继承关系如下：（菱形继承）.png)
...
![类相关13_类之间的继承关系](C++图库/类相关13_类之间的继承关系.png)
...
## 参考资料
参考《Effective C++》、《More Effective C++》、《Effective C++》、《Visual C++ 代码参考与技巧大全》、《STL》源码剖析、《大话设计模式》、《设计模式:可复用面向对象软件的基础》整理。
